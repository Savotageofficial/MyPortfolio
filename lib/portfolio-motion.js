export function setupPortfolioMotion() {
  // DOM references and animation state.
  ...
  // Measure the pinned project sequence.
  function layout() {
    if (disposed) return;
    ...
    document.body.classList.remove("work-compact");
    ...
    if (enabled && stage.scrollHeight > stage.clientHeight + 3) {
      // Longer descriptions and tag lists get tighter spacing before we fall back.
      document.body.classList.add("work-compact");
    }
    if (enabled && stage.scrollHeight > stage.clientHeight + 3) {
      enabled = false;
      document.body.classList.remove("work-motion", "work-compact");
    }
    if (enabled) {
  ...
  // Scroll progress, hero parallax, and project transforms.
  function render() {
    scheduled = false;
  ...
  // Frame scheduling and project navigation.
  ...
  // Reveal effects and reduced-motion handling.
  ...
  // Page events and active navigation links.
