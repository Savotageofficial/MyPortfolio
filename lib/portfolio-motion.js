export function setupPortfolioMotion() {
  ...
  function render() {
    ...
    if (!reduced.matches) {
      const hero = $(".hero"),
        p = clamp(scrollY / Math.max(hero.offsetHeight, 1));
      ...
      $(".marquee-track").style.transform = `translateX(${-scrollY * 0.13}px)`;
    }
    if (enabled) {
  ...
  function setupMotion() {
    ...
    if (reduced.matches) {
      ...
      document
        .querySelectorAll(".name-line,.hero-details,.hero-rule,.marquee-track")
