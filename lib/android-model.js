...
export function applyAndroidPose(
  ...
  const lift = reducedMotion
    ? 0
    : smooth((p - 0.1) / 0.2) * (1 - smooth((p - 0.78) / 0.17));
  const waveProgress = Math.max(0, Math.min(1, (p - 0.3) / 0.48));
  // Ease into two relaxed waves instead of starting and stopping at full speed.
  const envelope = Math.sin(waveProgress * Math.PI) ** 2;
  const wave = Math.sin(waveProgress * Math.PI * 4) * envelope;
  ...
  model.head.rotation.set(0, 0, -lift * 0.035);
  model.arms[0].rotation.z = 0.08;
  // A small forward shoulder angle gives the gesture depth without turning the robot away.
  model.arms[1].rotation.x = -lift * 0.12;
  model.arms[1].rotation.z = -0.08 + lift * (2.22 + wave * 0.22);
}
